...
namespace UnityStandardAssets.Vehicles.Aeroplane
{
	...
	public class AeroplaneController : MonoBehaviour
	{
		...
		private void KnowledgeBase()
		{
			...
			pitch.Add("TooMuchUpside", -mediumPitch);
			pitch.Add("TooMuchDownside", mediumPitch);
			pitch.Add("TooMuchUpside", -lowPitch);
			pitch.Add("TooMuchDownside", lowPitch);

			force.Add("Prestissimo", 2.5f);
			force.Add("Presto", 1.5f);
			force.Add("Alegro", 0.8f);
			force.Add("Piano", 0.3f);
		}

		private void Rules()
		{
			if ((PitchAngle <= pitch["TooMuchUpside"] && RollAngle <= roll["TooMuchLeft"]) ||
				(PitchAngle <= pitch["TooMuchUpside"] && RollAngle >= roll["TooMuchRight"]) ||
		...
		private void FuzzyController()
		{
			if (PitchInput == 0f && RollInput == 0f) {

				if (PitchAngle < 0 && PitchAngle > -maxPitch) {
					PitchInput = -PitchAngle * 1.5f;
				} else if (PitchAngle > 0 && PitchAngle < maxPitch) {
					PitchInput = -PitchAngle * 1.5f;
				...
				if (RollAngle < 0 && RollAngle > -maxRoll) {
					RollInput = -RollAngle * 0.5f;
				} else if (RollAngle > 0 && RollAngle < maxRoll) {
					RollInput = -RollAngle * 0.5f;
				} else if (RollAngle <= -maxRoll) {
					RollInput = -RollAngle * 0.5f;
				} else if (RollAngle >= maxRoll) {
					RollInput = -RollAngle * 0.5f;
				}
